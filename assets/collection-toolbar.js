...
(function () {
  ...
  */
  function initActiveFiltersNav() {
    ...
    // Only show the nav arrows (and the edge fade, toggled via the
    // --scrollable class) when the pills + Clear all actually overflow
    // the strip. With few active filters everything fits and neither
    ...
    function updateOverflowState() {
      ...
      bar.classList.toggle('collection-active-filters--scrollable', hasOverflow);
    }
    ...
    // Re-check on resize (e.g. rotating a tablet, or resizing a
    // desktop window) — bind once per bar element since this function
    // itself gets re-run after every AJAX filter swap.
    if (!bar.dataset.resizeBound) {
      bar.dataset.resizeBound = 'true';
